...
if __name__ == "__main__":
    EternalworldOL_followers_file = os.path.join(os.path.dirname(
        __file__), "EternalworldOL_followers.json")
    ...
    outFilePath = os.path.join(os.path.dirname(
        __file__), "EternalWorld_Active_Result.csv")
    ...
    cyberpop_followers = set(json.load(open(EternalworldOL_followers_file, 'r', encoding='utf-8')).values())
    print(f"EternalWorld twitter followers: {len(cyberpop_followers)}")
    ...
    columns = ["Timestamp", "Email Address", "Twitter Account Handle", "Discord Account Handle", "Wallet Address (bep20)"]
